# Updated for neurogym 2.2.0 / gymnasium compatibility:
# - Removed `import gym` (not needed)
# - Changed `ngym.TrialEnv` -> `TrialEnv` (from neurogym.core)
# - Added `from neurogym.core import TrialEnv`
# - Fixed _step() returns to use 5-tuple (gymnasium API): (ob, reward, terminated, truncated, info)
...
class _Reach(EnvWithAdditions):
    ...
    def __init__(self, dt=100, anti=True, rewards=None, timing=None,
                 dim_ring=16, reaction=False, integrate=0, seq=0, dly=False, sigma=1.):
    ...
    def _new_trial(self, **kwargs):
        ...
        if not self.reaction:
            ...
            self.add_randn(0, self.sigma, 'decision', where='stimulus')
...
# Dictionary of all tasks for easy import
TASKS = {
    # Go family
    'go': go, 'goseqr': goseqr, 'goseql': goseql,
    'anti': anti, 'antiseqr': antiseqr, 'antiseql': antiseql,
    'rtgo': rtgo, 'rtgoseqr': rtgoseqr, 'rtgoseql': rtgoseql,
    'rtanti': rtanti, 'rtantiseqr': rtantiseqr, 'rtantiseql': rtantiseql,
    'dlygo': dlygo, 'dlygoseqr': dlygoseqr, 'dlygoseql': dlygoseql,
    'dlygointr': dlygointr, 'dlygointl': dlygointl, 'dlygointseq': dlygointseq,
    'dlyanti': dlyanti, 'dlyantiseqr': dlyantiseqr, 'dlyantiseql': dlyantiseql,
    'dlyantiintr': dlyantiintr, 'dlyantiintl': dlyantiintl, 'dlyantiintseq': dlyantiintseq,
    # DM family
    'dm1': dm1, 'dm1seqr': dm1seqr, 'dm1seql': dm1seql,
    'dm2': dm2, 'dm2seqr': dm2seqr, 'dm2seql': dm2seql,
    'ctxdm1': ctxdm1, 'ctxdm1seqr': ctxdm1seqr, 'ctxdm1seql': ctxdm1seql,
    'ctxdm2': ctxdm2, 'ctxdm2seqr': ctxdm2seqr, 'ctxdm2seql': ctxdm2seql,
    'multidm': multidm, 'multidmseqr': multidmseqr, 'multidmseql': multidmseql,
    # DlyDM family
    'dlydm1': dlydm1, 'dlydm1seqr': dlydm1seqr, 'dlydm1seql': dlydm1seql,
    'dlydm1intr': dlydm1intr, 'dlydm1intl': dlydm1intl, 'dlydm1intseq': dlydm1intseq,
    'dlydm2': dlydm2, 'dlydm2seqr': dlydm2seqr, 'dlydm2seql': dlydm2seql,
    'dlydm2intr': dlydm2intr, 'dlydm2intl': dlydm2intl, 'dlydm2intseq': dlydm2intseq,
    'ctxdlydm1': ctxdlydm1, 'ctxdlydm1seqr': ctxdlydm1seqr, 'ctxdlydm1seql': ctxdlydm1seql,
    'ctxdlydm1intr': ctxdlydm1intr, 'ctxdlydm1intl': ctxdlydm1intl, 'ctxdlydm1intseq': ctxdlydm1intseq,
    'ctxdlydm2': ctxdlydm2, 'ctxdlydm2seqr': ctxdlydm2seqr, 'ctxdlydm2seql': ctxdlydm2seql,
    'ctxdlydm2intr': ctxdlydm2intr, 'ctxdlydm2intl': ctxdlydm2intl, 'ctxdlydm2intseq': ctxdlydm2intseq,
    'multidlydm': multidlydm, 'multidlydmseqr': multidlydmseqr, 'multidlydmseql': multidlydmseql,
    'multidlydmintr': multidlydmintr, 'multidlydmintl': multidlydmintl, 'multidlydmintseq': multidlydmintseq,
    # Match family
    'dms': dms, 'dmsintr': dmsintr, 'dmsintl': dmsintl,
    'dnms': dnms, 'dnmsintr': dnmsintr, 'dnmsintl': dnmsintl,
    'dmc': dmc, 'dmcintr': dmcintr, 'dmcintl': dmcintl,
    'dnmc': dnmc, 'dnmcintr': dnmcintr, 'dnmcintl': dnmcintl,
    'dmsseqr': dmsseqr, 'dmsseql': dmsseql, 'dmsintseq': dmsintseq,
    'dnmsseqr': dnmsseqr, 'dnmsseql': dnmsseql, 'dnmsintseq': dnmsintseq,
    'dmcseqr': dmcseqr, 'dmcseql': dmcseql, 'dmcintseq': dmcintseq,
    'dnmcseqr': dnmcseqr, 'dnmcseql': dnmcseql, 'dnmcintseq': dnmcintseq,
}

# =============================================================================
# Task categories for condition filtering
# =============================================================================

# Go family: tasks using _Reach class
# All have ground_truth (direction 0-15)
GO_TASKS = [
    'go', 'goseqr', 'goseql',
    'anti', 'antiseqr', 'antiseql',
    'rtgo', 'rtgoseqr', 'rtgoseql',
    'rtanti', 'rtantiseqr', 'rtantiseql',
    'dlygo', 'dlygoseqr', 'dlygoseql',
    'dlygointr', 'dlygointl', 'dlygointseq',
    'dlyanti', 'dlyantiseqr', 'dlyantiseql',
    'dlyantiintr', 'dlyantiintl', 'dlyantiintseq',
]

# Go family integration tasks - have randomly sampled delays from [0, 100, ..., 1100]
GO_DELAY_TASKS = [
    'dlygointr', 'dlygointl', 'dlygointseq',
    'dlyantiintr', 'dlyantiintl', 'dlyantiintseq',
]

# DM family: tasks using _DMFamily class (decision-making / 2AFC)
# No single ground_truth - target determined by coherence comparison
# Integration variants have FIXED delay=500 (not randomly sampled)
DM_TASKS = [
    'dm1', 'dm1seqr', 'dm1seql',
    'dm2', 'dm2seqr', 'dm2seql',
    'ctxdm1', 'ctxdm1seqr', 'ctxdm1seql',
    'ctxdm2', 'ctxdm2seqr', 'ctxdm2seql',
    'multidm', 'multidmseqr', 'multidmseql',
]

# DlyDM family: _DMFamily with delay comparison
# Integration variants have FIXED delay=500 (not randomly sampled)
DLYDM_TASKS = [
    'dlydm1', 'dlydm1seqr', 'dlydm1seql',
    'dlydm1intr', 'dlydm1intl', 'dlydm1intseq',
    'dlydm2', 'dlydm2seqr', 'dlydm2seql',
    'dlydm2intr', 'dlydm2intl', 'dlydm2intseq',
    'ctxdlydm1', 'ctxdlydm1seqr', 'ctxdlydm1seql',
    'ctxdlydm1intr', 'ctxdlydm1intl', 'ctxdlydm1intseq',
    'ctxdlydm2', 'ctxdlydm2seqr', 'ctxdlydm2seql',
    'ctxdlydm2intr', 'ctxdlydm2intl', 'ctxdlydm2intseq',
    'multidlydm', 'multidlydmseqr', 'multidlydmseql',
    'multidlydmintr', 'multidlydmintl', 'multidlydmintseq',
]

# Match family: tasks using _DelayMatch1DResponse class
# ground_truth is 'match' or 'non-match' (string, not integer)
MATCH_TASKS = [
    'dms', 'dmsintr', 'dmsintl',
    'dnms', 'dnmsintr', 'dnmsintl',
    'dmc', 'dmcintr', 'dmcintl',
    'dnmc', 'dnmcintr', 'dnmcintl',
    'dmsseqr', 'dmsseql', 'dmsintseq',
    'dnmsseqr', 'dnmsseql', 'dnmsintseq',
    'dmcseqr', 'dmcseql', 'dmcintseq',
    'dnmcseqr', 'dnmcseql', 'dnmcintseq',
]

# Match family integration tasks - have randomly sampled delays from [0, 100, ..., 1100]
MATCH_DELAY_TASKS = [
    'dmsintr', 'dmsintl', 'dmsintseq',
    'dnmsintr', 'dnmsintl', 'dnmsintseq',
    'dmcintr', 'dmcintl', 'dmcintseq',
    'dnmcintr', 'dnmcintl', 'dnmcintseq',
]

# =============================================================================
# Tasks with randomly sampled delays (suitable for delay holdout)
# =============================================================================
# These tasks sample delay from np.arange(0, 1200, 100) = [0, 100, ..., 1100]
DELAY_TASKS = set(GO_DELAY_TASKS + MATCH_DELAY_TASKS)

# =============================================================================
# Default delay splits for generalization testing
# =============================================================================
# All delays: [0, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100]
DEFAULT_TRAIN_DELAYS = [0, 100, 200, 300, 400, 500, 600, 700, 800]  # 9 values
DEFAULT_TEST_DELAYS = [900, 1000, 1100]  # 3 held-out values


def get_condition_filter(task_name: str, split: str = 'train'):
    """Get condition filter for a task based on split type.
    
    Args:
        task_name: Name of the task
        split: 'train', 'test', or 'all'
            - 'train': Filter to training conditions (delays 0-800 for delay tasks)
            - 'test': Filter to held-out conditions (delays 900-1100 for delay tasks)
            - 'all': No filtering
    
    Returns:
        condition_filter dict or None
    """
    if split == 'all':
        return None
    ...
    #     if split == 'train':
    #         return {'delay': DEFAULT_TRAIN_DELAYS}
    #     elif split == 'test':
    #         return {'delay': DEFAULT_TEST_DELAYS}
    #     else:
    #         raise ValueError(f"Unknown split: {split}. Use 'train', 'test', or 'all'")
    # breakpoint()
    # Task doesn't have configurable conditions
    return None