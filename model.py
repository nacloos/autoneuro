...
@struct.dataclass
class LSTMParams:
    """Parameters for LSTM module.

    Inspired by biological memory systems with separate storage (cell state)
    and gating (forget/input/output gates) - analogous to hippocampal
    encoding, consolidation, and retrieval gates.

    f_t = sigmoid(W_f @ [h_{t-1}, x_t] + b_f)   -- forget gate
    i_t = sigmoid(W_i @ [h_{t-1}, x_t] + b_i)   -- input gate
    c_hat = tanh(W_c @ [h_{t-1}, x_t] + b_c)     -- candidate cell
    c_t = f_t * c_{t-1} + i_t * c_hat             -- cell update
...
def lstm_step(params: LSTMParams, state, x: jnp.ndarray):
    """Single step of LSTM module. State is (h, c) tuple."""
    h, c = state
    hx = jnp.concatenate([h, x])

    f = jax.nn.sigmoid(params.W_f @ hx + params.b_f)   # forget gate
    i = jax.nn.sigmoid(params.W_i @ hx + params.b_i)   # input gate
    c_hat = jnp.tanh(params.W_c @ hx + params.b_c)     # candidate
    c_new = f * c + i * c_hat                            # cell update
    o = jax.nn.sigmoid(params.W_o @ hx + params.b_o)   # output gate
    h_new = o * jnp.tanh(c_new)                          # hidden state

    y = params.W_out @ h_new
    return (h_new, c_new), y, jnp.mean(f)  # return mean forget gate as diagnostic


def init_lstm_params(rng, input_dim, output_dim, h_dim):
    """Initialize LSTM module parameters."""
    keys = jax.random.split(rng, 5)
    concat_dim = h_dim + input_dim
    scale = (2.0 / concat_dim) ** 0.5
    return LSTMParams(
        W_f=jax.random.normal(keys[0], (h_dim, concat_dim)) * scale,
        b_f=jnp.ones(h_dim),  # Initialize forget gate bias to 1 (keep memories by default)
